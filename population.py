...
def sanitize_output(usputput, # unsanitized output from ANN
    ...
    for i in range(max_pegs):
        ...
        #   still dont.
       soutput[i] = int( np.argmax( usputput[0, max_dif_pegs*i:max_dif_pegs*(i+1)] )
                         +1 )
...
if __name__ == '__main__':
    ...
    meep1:Meeple = Meeple(input_size=inputsize,
                         hidden_size=hiddensize,
                         output_size=outputsize,
                         isHallow=False)
    meep2:Meeple = Meeple(input_size=inputsize,
                         hidden_size=hiddensize,
                         output_size=outputsize,
                         isHallow=False)
    ...
    #specie = Species(0, meep1 )

    #print( meep1.brain.getAmountWeights() )
    #print( meep2.brain.getAmountWeights() )
    #print( specie.getAmtSimilarGenes(meep1, meep2) )

    for ai in range(2):#max_attempts):
        sinput = sanitize_input(meep1.results_list, max_pegs, max_attempts)
        output = meep1.brain.predict(sinput)
        print(output)
        attempt = sanitize_output(output, max_pegs, max_dif_pegs)
        result = check_attempt(attempt, mastermind_solution)
        meep1.results_list.append(np.array([attempt, result]))
        print(ai)
